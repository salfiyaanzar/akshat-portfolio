      ...
      <div className="grid sm:grid-cols-2 gap-3 sm:gap-3">
        {PROJECTS.map((p) => (
          <a
            ...
            className={[
              'group flex flex-col gap-2 p-4 sm:p-5 rounded-xl bg-neutral-50 dark:bg-white/[0.02] backdrop-blur-md border-[1.5px] dark:border border-neutral-200 dark:border-white/10 shadow-lg hover:bg-neutral-100 dark:hover:bg-white/[0.04] hover:border-neutral-300 dark:hover:border-white/20 transition-all duration-300',
              p.wide ? 'sm:col-span-2' : '',
            ].join(' ')}
          >
            <div className="flex items-center justify-between">
              <h3 className="text-base sm:text-lg font-normal text-neutral-900 dark:text-white group-hover:text-neutral-700 dark:group-hover:text-neutral-200 transition-colors">
                {p.title}
              </h3>
              <ArrowUpRight className="w-5 h-5 text-neutral-500 opacity-0 group-hover:opacity-100 transition-opacity" strokeWidth={1.5} />
            </div>
            <p className="text-sm sm:text-base text-neutral-600 dark:text-neutral-400 leading-normal font-normal">
              {p.description}
            </p>
          </a>
        ))}
